...
The template is built on three strict layers:
...
- Server actions own UI writes (POST/mutations from our own UI). See [Read vs Write](/docs/architecture/read-vs-write) for the full breakdown of why.
...
place to house the code. Business logic never bleeds into route handlers, data access never
appears in components. That makes the codebase predictable and boring, so you always know where
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `app/`                       | Next.js file-convention files only   | `page.tsx`, `layout.tsx`, `route.ts` etc. — thin shells that delegate to `components/` and `classes/`. No business logic. |
...
| `components/ui/`             | shadcn primitives                    | Vendored — never edit directly                                                                                            |
...
| `components/providers/`      | Client-side providers                | Compose all providers here; mount in `layout-client.tsx`                                                                  |
...
| `classes/loggers/`           | Logger classes                       | Cross-cutting; not server-only                                                                                            |
...
| `utils/`                     | Shared utility functions             | Pure functions; co-locate tests in `__tests__/`                                                                           |
