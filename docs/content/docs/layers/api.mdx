...
- **`Admin`** - gets into the app and can **read** (`withAuth`).
- **`SuperAdmin`** - a **strict superset**: everything Admin can do, **plus
  writes** (`withRole('SuperAdmin')`). A SuperAdmin therefore also satisfies
  `withRole('Admin')`.
...
schema _and_ the same role gate so they cannot drift.
