...
Pure helper functions that aren't React concepts follow a scope rule:

| Scope                                        | Where                                  | Example                                          |
| -------------------------------------------- | -------------------------------------- | ------------------------------------------------ |
| Shared across the whole app, domain-agnostic | `src/utils/`                           | `formatCurrency`, `cn()`, `formatErrorMessage`   |
| Specific to one feature                      | `components/features/<feature>/utils/` | Invoice row formatting helpers                   |
| Stateful / reusable across components        | `hooks/` — extract to a hook           | Anything using `useState`, `useEffect`, `useRef` |

`src/utils/` carries an implicit contract: **safe to use anywhere**. Don't put feature-specific
helpers there — if it only makes sense in the context of one feature, it's not a shared utility.
